...
<head>
    ...
    <title>마이페이지 - 인테리어 도면 다운로드</title>
    ...
    <style>
        ...
        .board-table .action-buttons { /* download-buttons에서 action-buttons로 변경 */
            display: flex;
            gap: 8px;
        }
        ...
        .board-table .btn-download:hover {
...
<%@ include file="/WEB-INF/views/include/top.jsp" %>
    ...
    <div class="mypage-wrapper"> 
        ...
        <main class="main-content"> <div class="mypage-header">
                <h2>인테리어 도면 다운로드 🏠</h2>
                <p>회원님이 제작하거나 저장한 인테리어 도면을 이곳에서 확인하고 관리하세요.</p>
            </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        downloadButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                const fileName = row.children[1].textContent; // Get the file name
                alert(fileName + '을(를) 다운로드합니다.');
                // In a real application, you would trigger the actual file download here
                // e.g., window.location.href = 'path/to/your/download/' + fileName + '.pdf';
            });
        });
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                const fileName = row.children[1].textContent; // Get the file name from the second column
                if (confirm(fileName + '을(를) 정말로 삭제하시겠습니까?')) {
